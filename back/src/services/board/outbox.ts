...
import { getCard, hasLiveSession, type Card } from "./registry.js";
...
/**
 * Read-only probe of a card's tmux session. `|| true` on purpose: a session that does not exist, a
 * tmux server that is not running and a container that is down all mean the same thing here — the
 * agent is not reachable — and none of them is an error worth a stack trace. PURE.
 */
export function buildAgentProbeScript(containerName: string, tmuxSession: string): string {
  return (
    `docker exec ${shQuote(containerName)} tmux list-panes -t ${shQuote(tmuxSession)} ` +
    `-F '#{pane_current_command}' 2>/dev/null || true`
  );
}

/** What the probe's output means. Empty = no session; any non-shell pane = the agent is up. PURE. */
